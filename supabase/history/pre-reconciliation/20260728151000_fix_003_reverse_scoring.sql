-- FIX-003 — Aplica reverse_scored às agregações e à importação de riscos.
-- Estado anterior: todas as respostas eram lidas no mesmo sentido.
-- Estado posterior: itens reverse_scored usam (6 - valor) na escala atual 1..5.
-- Dependências: assessment_cycles, questionnaire_sections,
-- questionnaire_items, assessment_responses, risk_items, risk_audit_log.
-- Impacto: médias e classificações podem mudar; executar primeiro em staging.
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    WHERE e.id = p_establishment_id
      AND e.tenant_id = v_cycle.tenant_id
  ...
  IF p_department_id IS NOT NULL AND NOT EXISTS (
    ...
    WHERE d.id = p_department_id
      AND d.establishment_id = p_establishment_id
      AND d.tenant_id = v_cycle.tenant_id
  ...
  FROM (
    ...
    WHERE qs.template_id = v_cycle.questionnaire_template_id
      AND ai.establishment_id = p_establishment_id
...
BEGIN
  ...
  FOR v_dim IN
    ...
    WHERE ar.cycle_id = p_cycle_id
      AND ar.tenant_id = v_cycle.tenant_id
...
-- Testes positivos: escala 1..5, item normal e reverso devem convergir para a
-- mesma direção de risco; owner/admin/manager do tenant podem importar.
-- Testes negativos: abaixo do limiar, tenant diferente, anônimo, viewer e
-- segunda importação não devem criar riscos.
