-- =============================================================================
-- SEC-BLOCK1-CONSOLIDATION v1.2.2 — EXPAND PHASE
-- Estrategia expand–migrate–contract para zero downtime.
--
-- EXPAND: Cria funcoes _v2 com novas assinaturas, MANTEM funcoes antigas.
--         Nenhuma funcao existente e removida ou alterada de forma incompativel.
--         Funcoes sem mudanca de assinatura recebem CREATE OR REPLACE seguro.
--
-- Funcoes _v2 criadas (novas assinaturas, service_role only):
--   - fn_check_pin_rate_limit_v2(text, text, int, int, int) — dual rate limit
--   - fn_access_complaint_v2(text, text, text) — IP hash via gateway
--   - fn_send_reporter_message_v2(text, text, text, text) — IP hash via gateway
--
-- Funcoes novas (sem predecessora):
--   - fn_record_pin_failure(text, text) — registro separado de falha
--
-- Funcoes atualizadas (mesma assinatura, body change via CREATE OR REPLACE):
--   - fn_submit_complaint — PIN min 6, validacao numerica, enum casts
--   - check_plan_limit(text) — multi-org check, delega para overload
--   - check_plan_limit(uuid, text) — overload nova para multi-org
--   - fn_remove_member — tenant derivado do alvo, advisory lock
--   - fn_get_complaint_list (3-arg e 4-arg) — paginacao correta, multi-org
