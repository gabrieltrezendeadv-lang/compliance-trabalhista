-- FIX-004 — Valida integralmente a submissão pública de avaliações.
-- Estado anterior: token sem lock, JSON sem validação, itens alheios aceitos e
-- ausência de verificação de expiração/fim do ciclo.
-- Estado posterior: lock transacional, ciclo vigente, itens únicos do template,
-- escala 1..5 e todas as questões obrigatórias presentes.
-- Dependências: assessment_invitations, assessment_cycles,
-- questionnaire_sections, questionnaire_items, assessment_responses.
...
BEGIN
  ...
  BEGIN
    IF EXISTS (
      ...
      LEFT JOIN public.questionnaire_items qi
        ON qi.id = (x->>'item_id')::uuid
      LEFT JOIN public.questionnaire_sections qs
        ON qs.id = qi.section_id
  ...
  SELECT count(*)
  INTO v_submitted_required_count
  FROM jsonb_array_elements(v_response) x
...
-- Testes positivos: token válido, ciclo ativo e payload completo 1..5.
-- Testes negativos: token inválido/expirado/usado, ciclo encerrado, JSON inválido,
-- item duplicado, item de outro template, valor fora da escala e concorrência.
